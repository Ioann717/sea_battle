        ...
        for ship in self.ships:
            if ship.shooten(d):
                ...
                if ship.lives == 0:
                    ...
                    print(color.RED + "Корабль врага уничтожен!" + color.END)
                    return False
                else:
                    print(color.RED + "Корабль врага повреждён!" + color.END)
                    return True

        self.field[d.x][d.y] = color.RED + "✸" + color.END
        print("Промах! Ну ничего..")
        return False
...
class User(Player):
    def ask(self):
        while True:
            cords = input(color.PURPLE + "Ваш ход, месье: " + color.END).split()
